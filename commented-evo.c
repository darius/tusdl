...
enum {
  ...
  thumb_width     = tile_width * thumb_cols,
  thumb_height    = tile_height * thumb_rows,
...
/* Return a file open for writing whose name is generated from
   ...
   FIXME The implementation has an obvious race condition, but I'm declaring
...
/* Random numbers.
   I think it turned out to be important to have a faster random number
   generator than the stdlib one. */
...
/* 'grid' is the array of 'screen's pixels, 4 bytes each. */
static SDL_Surface *screen = NULL;
static Pixel *grid;

/* Initialize SDL, setting screen and grid. */
static void
start_sdl (void)
{
  ...
  grid = (Pixel *) screen->pixels;

  /* Ignore mouse motion events */
  SDL_EventState (SDL_MOUSEMOTION, SDL_IGNORE);
}

/* Set the pixel at (x,y) to 'value'.  The change will be visible after
   the next call to show(). */
static INLINE void
put (int x, int y, Pixel value)
{
...
/* Push onto vm's stack an event value and event code:
        0 0 for no event,
        k 1 for keypress,
    (y,x) 2 for mouse click. */
static void
...
/* Wait for an event and report it as in event_adapter(). */
static void
...
/* Refresh the screen using the grid. */
static void
show (void)
{
...
/* Convert an intensity ([0..1] nominal range, but may spill out of those limits)
   to a clipped RGB byte. */  
static INLINE Uint8
...
/* Fill (dr,dg,db) with RGB color values taken by interpreting the corresponding
   (ar,ag,ab) pixel values as HWB colors.  (I think.  I'd need to dig up my original
   source for this code to be sure.)
   The HWB color space is some random space meant to be 'intuitive' -- I was hoping
   that'd bring out striking colors, and it worked pretty well.  I think it deserves
   a *lot* of the credit for this program's appeal compared to other genetic art 
   thingies I've seen. */
static void
...
/* Fill dest with random 1-bit values, on with probability proportional to 'a'. */
static void
op_sprinkle (Intensity *dest, Intensity *a)
{
  FOR_EACH (x, y, j)
    dest[j] = (my_rand () / (double)UINT_MAX < a[j] ? 1.0 : 0.0);
}
...
#define unop(name, expr)                               \
  ...
  {                                                    \
    FOR_EACH (x, y, j)                                 \
      {                                                \
	Intensity arg1 = a[j]; dest[j] = expr;         \
      }                                                \
  }
...
#define binop(name, expr)                                      \
  ...
  {                                                            \
    FOR_EACH (x, y, j)                                         \
      {                                                        \
	Intensity arg1 = a[j], arg2 = b[j]; dest[j] = expr;    \
      }                                                        \
  }
...
binop (op_mix, (my_rand () & 1) ? arg1 : arg2)
...
struct Node {
  ...
  int step;			/* program-counter for this operation in the uncompiled */
				/* program (only used by mix and sprinkle ops) */
  char *name;
  unsigned hashcode;
  Node *next;			/* The next node in the hashtable bucket. */
  Intensity *result;		/* The computed intensity field, or NULL if not yet computed. */
};
...
evaluate (Node *node, Coord_system cs, int col, int row)
{
  ...
  if (cs == small)
    {
      left    = -1.5 + (3.0/thumb_cols) * col;
      top     = -1.0 + (2.0/thumb_rows) * row;
      x_scale = 3.0/thumb_width;
      y_scale = 2.0/thumb_height;
      tile_id = row * thumb_cols + col;
    }
  else if (cs == big)
    {
      left    = -1.5 + (3.0/(cols*thumb_cols)) * col;
      top     = -1.0 + (2.0/(rows*thumb_rows)) * row;
      x_scale = 3.0/(tile_width*cols*thumb_cols);
...
output_picture (FILE *out)
{
  ...
  fprintf (out, "%d %d 255\n", grid_width, grid_height);
  {
	...
	for (j = 0; j < grid_width; ++j) 
	  {
	    Pixel pixel = grid[i * grid_width + j];
...
install_evo_words (ts_VM *vm)
{
  ...
  ts_install (vm, "start-sdl",       ts_call_void_0,  (int) start_sdl);
  ts_install (vm, "clear",           ts_call_void_0,  (int) clear);
  ts_install (vm, "show",            ts_call_void_0,  (int) show);
  ts_install (vm, "listen",          listen,          0);
  ts_install (vm, "wait",            blocking_listen, 0);

  ts_install (vm, "populate",        ts_call_void_2,  (int) populate);
  ts_install (vm, "mutate",          ts_call_void_2,  (int) sample);
  ts_install (vm, "copy",            ts_call_void_4,  (int) copy);
  ts_install (vm, "generate",        ts_call_void_2,  (int) generate);
  ts_install (vm, "generate-big",    ts_call_void_4,  (int) generate_big);
  ts_install (vm, "complexity",      ts_call_int_2,   (int) complexity);

  ts_install (vm, "save-image",      ts_call_void_0,  (int) save_image);
  ts_install (vm, "append",          ts_call_void_0,  (int) append);
  ts_install (vm, "save",            ts_call_void_0,  (int) save);
  ts_install (vm, "restore",         ts_call_void_0,  (int) restore);
}
