...
enum { ts_stack_size = 100 };	/* Max. depth of the data stack */
enum { ts_data_size = 8192 };	/* Max. # of bytes in the data area */
				/*  (must be a multiple of sizeof(int) */
enum { ts_dictionary_size = 512 }; /* Max. # of dictionary entries */
...
typedef void ts_ErrorFn (ts_VM *vm, const char *format, va_list args);
...
struct ts_VM {
  ...
  int here;			/* The next free cell within data[] */
...
void ts_install_float_words (ts_VM *vm);
